...
// 4. Activation Edge: CCyNP/CCyP	
	TIMx->CCER &= ~ (10 << (4 * (ICn-1)));						// CCy(Rising) for ICn
...
// Configure Activation Edge direction
	...
	switch(edge_type){
		case RISE_TIM: TIMx->CCER &= ~( 10 << 4 * (ICn-1));	 break; //rising:  00
		case FALL_TIM: TIMx->CCER |= 2<<( 4 * (ICn-1));	 break; //falling: 01
		case BOTH_TIM: TIMx->CCER |= 10 <<(4 * (ICn-1));	 break; //both:    11
...
void ICAP_pinmap(IC_t *timer_pin){
   ...
   if(port == GPIOA) {
      ...
      switch(pin){
         ...
         case 0: timer_pin->timer = TIM3; timer_pin->ch = 3; break;
         case 1: timer_pin->timer = TIM3; timer_pin->ch = 4; break; 
